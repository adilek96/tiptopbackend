...
type ImportBody = {
  ...
  source_site?: string
  raw_text?: string
  /** Вызов API напрямую: только ссылки, файлы качает сервер. */
  image_urls?: string[]
  video_url?: string
 ...
 *
 * Запасной путь для вызова API со списком ссылок: расширение присылает
 * файлы уже скачанными браузером и сюда не заходит. Referer подставляется
 * от исходной страницы — CDN Taobao и Alibaba отдают картинки только со
 * «своим» реферером, а без него возвращают заглушку. Из датацентра даже
 * это срабатывает не всегда, поэтому импорт из расширения надёжнее.
 */
 ...
 *
 * Основной клиент — расширение для Chrome: оно присылает всё разом, включая
 * скачанные браузером фото и настоящие варианты продавца.
 *
 * Поля image_urls и video_url оставлены для вызова API напрямую — скриптом
 * или с площадки, которую расширение не знает. Тогда файлы качает сервер, а
 * варианты восстанавливает модель по тексту. Это заметно менее надёжно:
 * CDN площадок часто не отдают картинки запросу из датацентра.
 *
